...
\begin{abstract}
Large language models remain critically dependent on prompt quality, and automatic prompt optimization (APO) methods often produce output that lacks the structured guidance human-crafted prompts provide. This paper investigates Meta-Prompted Instruction Refinement (MPIR), a lightweight, model-agnostic layer that refines an APO-generated prompt through a seven-criteria rubric, meta-prompted evaluation and refinement, and empirical validation against a held-out set. An initial evaluation, using GPT-4o to judge and refine prompts targeting GPT-3.5-turbo, reported per-task gains on a majority of Big-Bench Hard (BBH) tasks but a pooled improvement that did not reach conventional significance; a subsequent audit found its validation stage had scored candidates against data the upstream optimizer had already trained on, so even its per-task pattern was not reliable on its own. We corrected the split and asked the question the initial evaluation could not answer cleanly: were the apparent gains attributable to the rubric-refinement mechanism, or to GPT-4o's capability advantage over the model it refined prompts for? To isolate the mechanism, we reran a pre-registered go/no-go pilot with a single small open-weight model (Qwen3-1.7B) acting as target, optimizer, and refinement judge simultaneously, across three APO backbones (PromptWizard, APE, ProTeGi) on the three BBH tasks where the original evaluation showed its largest gains. The effect did not reproduce: MPIR's refined prompt was byte-identical to its base optimizer's prompt in all nine (task, optimizer) cells tested---refinement was attempted every time and accepted zero times---and pooled McNemar and GEE tests on the resulting predictions return p-values of 1.0 (or undefined, where accuracy did not vary at all) and accuracy differences of 0.0000 across all three optimizers. Tracing the cause, we identify and fix a general defect in the refinement algorithm itself---it never scores the pre-refinement prompt as a baseline, so any round is accepted as an "improvement" regardless of whether it beats doing nothing---plus six further correctness bugs surfaced by running the pilot live, two of them distinct ways a small model's meta-prompting output can be well-formed but semantically empty, each invisible until run against a live model rather than a mocked one. Correcting all seven, the layer still could not produce a single accepted refinement under a model too weak to reliably critique its own prompts. We report this as a boundary-condition finding rather than a positive result: a cheap, post-hoc rubric layer of this kind appears to depend on the refining model's own capability, and its benefit, if any, is smaller than the original per-task deltas alone would suggest.
\end{abstract}
...
\subsection{Illustrative Application of the Rubric}
...
The refinement meta-prompt in Figure 3 asks the LLM to return the revised prompt wrapped between <START> and <END> delimiters, which the implementation then extracts programmatically. A weaker meta-model does not always comply, and Section 5.2.4 reports two concrete ways it failed to during the pilot: no delimited candidate at all, or a well-formed delimited candidate whose content was itself a verbatim echo of a meta-prompting template the model had recently seen in its own context window (Appendix B), rather than a genuine refined instruction. The implementation guards against both before a candidate ever reaches validation: an empty extraction result falls back to the pre-refinement prompt unchanged, and a candidate containing verbatim phrases unique to MPIR's own evaluation template (Figure 2) is rejected on the same grounds. Neither check depends on recognizing a specific failure by name, and Section 3.2's baseline-scoring correction provides a further, more general safety net behind both: even a well-formed, uncontaminated candidate is only adopted if it empirically outperforms the prompt it was meant to improve.
...
Figure A.4 reproduces APE's actual optimized instruction for hyperbaton (abridged for space; unabridged in the project repository). Unlike PromptWizard's search (Figure A.1), APE's forward-mode generation \citep{zhou2023} asks the meta-model to reverse-engineer an instruction from input-output demonstrations, which is why the resulting prompt is itself phrased as a "what instruction produced these examples" question rather than a direct task instruction. This framing is the same one Appendix B's Figure B.1 shows a refinement attempt echoing verbatim instead of producing a genuinely refined instruction.
